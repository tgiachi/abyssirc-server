...
public class RplList : BaseIrcCommand
{
    ...
    public override void Parse(string line)
    {
        ...
        if (int.TryParse(parts[3], out var userCount))
        {
    ...
    {
        return string.IsNullOrEmpty(ServerName) ? string.IsNullOrEmpty(Topic)
                ? $"322 {Nickname} {ChannelName} {VisibleUserCount}"
                : $"322 {Nickname} {ChannelName} {VisibleUserCount} :{Topic}" :
            string.IsNullOrEmpty(Topic) ? $":{ServerName} 322 {Nickname} {ChannelName} {VisibleUserCount}" :
